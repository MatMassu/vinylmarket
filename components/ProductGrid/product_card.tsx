...
export default async function ProductCard({ product }: ProductCardProps) {
  return (
    <div className="flex flex-col gap-1 h-full">
      <div className="relative aspect-square md:hover:shadow-lg md:hover:-translate-y-1 transition-all duration-250 ease-out">
      ...
      <div className="flex flex-col text-xs gap-0.5">
        <Link href={`/vinyl/${product.id}`} scroll={false}>
          <h1 className="font-montserrat text-base line-clamp-1 leading-snug font-bold cursor-pointer text-neutral-800 md:hover:text-indigo-600">
            {product.title}
          </h1>
        </Link>
