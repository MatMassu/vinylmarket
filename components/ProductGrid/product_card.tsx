...
import { getFrontImages } from "../../lib/blob";
...
export default async function ProductCard({ product }: ProductCardProps) {
  const images = await getFrontImages(product.id);

  const gridImage = images.find((i) => i.variant === "grid");
  const cartImage = images.find((i) => i.variant === "cart");
  ...
  return (
    <div className="flex flex-col gap-1 h-full">
      <div className="relative aspect-square hover:shadow-lg hover:-translate-y-1 transition-all duration-250 ease-out">
        ...
        <AddToCartButton product={product} images={{ grid: gridImage.url, cart: cartImage.url }} />
      </div>
